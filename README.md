react_Indiana
夺宝项目 本项目是基于react实战项目。代码简单易懂。实现了移动端使用最多的 无限滚动，图片加载，左右滑动，登录退出。
项目技术架构
react + react-router + redux + immutable + less + ES6/7 + webpack + fetch

###安装
项目地址：（git clone）
git clone https://github.com/ziyewuli/react_Indiana.git 通过npm安装本地服务第三方依赖模块(需要已安装Node.js)
npm install 或使用淘宝镜像cnpm install 启动服务(http://localhost:8088)
npm run dev 发布代码
npm run build
###注意
